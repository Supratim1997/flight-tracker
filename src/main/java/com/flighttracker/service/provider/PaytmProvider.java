...
@Component
public class PaytmProvider implements FlightProvider {

    private final HttpClient httpClient;
    private final ObjectMapper objectMapper;

    public PaytmProvider() {
    ...
    public String getProviderName() {
        return AppConstants.PROVIDER_PAYTM;
    }
    ...
    public List<ScrapedFlight> searchFlights(String dep, String arr, LocalDate flightDate, String flightType) {
        ...
        String searchUrl = "https://paytm.com/flights/flight-search/" + dep.toUpperCase() + "/" + arr.toUpperCase() + "/1/0/0/e/" + formattedDate;
            ...
            if (pos != -1) {
                ...
                if (start != -1 && end > start) {
                    try {
                        ...
                        JsonNode flightList = root.path("props").path("pageProps").path("flights");
                        if (flightList.isArray()) {
                            for (JsonNode f : flightList) {
                                String airline = f.path("airline").asText("IndiGo");
                                String flightNo = f.path("flightNumber").asText("6E-201");
                                double price = f.path("price").asDouble(4450.0);
                                ...
                                flights.add(new ScrapedFlight(
                                        airline, flightNo, LocalTime.of(15, 30), LocalTime.of(17, 45),
                                        BigDecimal.valueOf(price), isDirect,
                                        isDirect ? AppConstants.DIRECT_STOPS_TEXT : "1 Stop",
                                        searchUrl, AppConstants.PROVIDER_PAYTM
                                ));
                            }
                        }
