...
@Component
public class AirIndiaDirectProvider implements FlightProvider {

    private final HttpClient httpClient;
    private final ObjectMapper objectMapper;

    public AirIndiaDirectProvider() {
    ...
    public String getProviderName() {
        return AppConstants.PROVIDER_AIR_INDIA_DIRECT;
    }
    ...
    public List<ScrapedFlight> searchFlights(String dep, String arr, LocalDate flightDate, String flightType) {
        ...
        String searchUrl = "https://www.airindia.com/in/en/book/flight-search.html?from=" + dep.toUpperCase() + "&to=" + arr.toUpperCase() + "&date=" + formattedDate;
        ...
        try {
            ...
            int pos = html.indexOf("window.airIndiaData =");
            if (pos != -1) {
                ...
                if (start != -1 && end > start) {
                    try {
                        ...
                        JsonNode flist = root.path("flights");
                        if (flist.isArray()) {
                            for (JsonNode f : flist) {
                                String flightNo = f.path("flightCode").asText("AI-865");
                                double price = f.path("fare").asDouble(4500.0);
                                ...
                                flights.add(new ScrapedFlight(
                                        "Air India", flightNo, LocalTime.of(10, 0), LocalTime.of(12, 15),
                                        BigDecimal.valueOf(price), isDirect,
                                        isDirect ? AppConstants.DIRECT_STOPS_TEXT : "1 Stop",
                                        searchUrl, AppConstants.PROVIDER_AIR_INDIA_DIRECT
                                ));
                            }
                        }
